...
for ii=1:numel(fields_vals)
    ...
    if strcmp(SPLIT{1},'ROIS')
        ...
        for kk=1:numel(var_fields.id)
            ...
            if ok_idx ~= 0
                ...
                var_fields.(var_name){kk} = NaN;
            end
        end
        var_fields.(var_name)=var_fields.(var_name)';
        var_fields.(var_name)=cell2mat(var_fields.(var_name))
    elseif strcmp(SPLIT{1},'TRKS')
        ...
        maxlen_trkname=strcat('maxlen',trk_prefix);
        numsstr_trkname=strcat('numsstr',trk_prefix);
        %Get IDs for each value:
        for kk=1:numel(tmp_TRKs)
            tmp_TRKS_ids{kk}=tmp_TRKs{kk}.id{end};
        end
        %Looping though all values and assigning them to each id
        for kk=1:numel(var_fields.id)
            ...
            if ok_idx ~= 0
                ...
                var_fields.(maxlen_trkname){kk} =tmp_TRKs{ok_idx}.num_uvox;
                var_fields.(numsstr_trkname){kk} =tmp_TRKs{ok_idx}.num_uvox;
            else
                var_fields.(vol_trkname){kk} =NaN;
                var_fields.(maxlen_trkname){kk} =NaN;
                var_fields.(numsstr_trkname){kk} =NaN;
            end
        end
        var_fields.(var_name)=var_fields.(var_name)';
        var_fields.(var_name)=cell2mat(var_fields.(var_name))
...
myTable=join(outTable,struct2table(var_fields));
