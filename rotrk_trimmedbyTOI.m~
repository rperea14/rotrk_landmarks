...
switch WHAT_TOI
    case 'postcing',
        for tohide=1:1
            ...
            for itrk=1:numel(trks_in.sstr)
                ...
                if tmp_val > roi_lim{1}(3)
                    tmpmax_itrk_idx=itrk;   %save the strline index
                    tmpmax_xyz_idx=tmp_idx; %save the strline xyz index
                    continue %this should exit the for loop. Not sure if it works :/
                end
            end
            flipped_trks_in = rotrk_flip(trks_in,trks_in.sstr(tmpmax_itrk_idx).matrix(tmpmax_xyz_idx,:));
            %INIT *;sstr fields:
 ...
 case 'fx',
        for tohide=1:1
            ...
            flipped_trks_in = rotrk_flip(trks_in,roi_mean{1});
            %INIT *;sstr fields:
            trks_out.header=flipped_trks_in.header;
            trks_out.header.specific_name=[ 'trimmed_' flipped_trks_in.header.specific_name ] ;
            trks_out.id=flipped_trks_in.id;
